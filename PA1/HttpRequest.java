				...
				if (tokens.length < 3) {
					...
					if (port == -1) {
						if (protocol.equals("http")) {
							...
							throw new Exception("Wrong protocol, please check");//http procotol only
						}
					}
					query = url.getQuery();
					...
					Socket newSocket = new Socket(host, port);//New socket to connect server
					...
					while ((line = rd.readLine()) != null) {
						response.append(line + "\r\n"); //receive and build response string from server
					}
					DataOutputStream toClient = new DataOutputStream(socket.getOutputStream());
					toClient.writeBytes(response.toString()); //Write response to client
					toClient.flush();
					toClient.close();
					to.close();
					try {
